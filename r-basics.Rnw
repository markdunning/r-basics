...
\notebell The order of arguments does not matter if you assign values to the names of arguments in a function. See the example below where the arguments of \Rfunction{runif} have been given in different ways. When giving values as arguments to a function, the order only matters when you have not specifically assigned the values to the names of the various possible arguments.
...
With knowledge of a few programming basics, we can save ourselves effort in typing. 
...
\textinfo It is trival to calculate the mean of each column manually in turn. However, this would be tedious for high-dimensional data. Moreover, if we needed to re-use the code on a different matrix, we would have modify our code every time.
...
To construct the loop we need to define a test condition and some R code to run for each \textit{iteration} (which can be as long as we like). In this example we want to 'loop' over each column and make a variable \Rcode{i} to represent the column that we are currently working with \footnote{Conventionally we often use variables named i, j,...etc when defining loops}. At each iteration, \Rcode{i} takes a particular value between the limits we have defined. The code is then executed by substituting our particular value of \Rcode{i}. In our case starting with 1 and ending with 10. The code below shows how the value of i changes as the code executes.
...
We now change to code to calculate the mean of each column
